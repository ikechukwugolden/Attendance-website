...
export default function Logs() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // 🟢 Handling the Index Error gracefully in UI
  ...
  return (
    <div className="space-y-8 pb-20 max-w-6xl mx-auto">
      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tighter italic">Analytics</h2>
          <p className="text-slate-400 font-bold text-sm">Monthly workforce behavior & export.</p>
        </div>
        <button 
          onClick={exportToCSV}
      ...
      {/* Monthly Calendar */}
      <div className="bg-white p-8 rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50">
          ...
          {getDaysInMonth(viewDate).map((date, idx) => {
            ...
            return (
              <div 
                ...
                className={`
                  aspect-square rounded-3xl flex flex-col items-center justify-center cursor-pointer transition-all border-2
                  ${selectedDate === dateStr ? 'bg-slate-900 border-slate-900 shadow-xl scale-110' : 'border-transparent hover:border-slate-100 hover:bg-slate-50'}
                  ${isToday && selectedDate !== dateStr ? 'bg-blue-50 border-blue-100' : ''}
                `}
              >
      ...
      <div className="bg-white rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden">
        <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row gap-6 items-center">
          ...
          <div className="bg-blue-600 text-white px-8 py-5 rounded-[2rem] font-black text-[10px] uppercase tracking-[0.2em] shadow-lg shadow-blue-200">
            {dailyLogs.length} Records Found {/* 🟢 FIXED: Changed total to dailyLogs.length */}
          </div>
        </div>
              ...
              {dailyLogs.length > 0 ? (
                ...
                <tr>
                  <td colSpan="3" className="py-24 text-center">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-300">No logs for this date</p>
                  </td>
                </tr>
              )}
            </tbody>
