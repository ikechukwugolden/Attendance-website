...
import { usePaystackPayment } from "react-paystack"; // 👈 Swapped Stripe for Paystack
import {
  Save, Clock, MapPin, Loader2, Target, Building2,
  Upload, X, ShieldCheck, Zap, ZapOff, Search, Layers, Crown
} from "lucide-react";
...
export default function Settings() {
  ...
  // --- 💰 PAYSTACK UPGRADE LOGIC ---
  const paystackConfig = {
    reference: `TRX-${Math.floor(Math.random() * 1000000000 + 1)}`,
    email: user?.email,
  ...
  const handleUpgrade = () => {
    ...
    initializePayment(
      async (reference) => {
        // Success Callback
        const toastId = toast.loading("Finalizing Pro status...");
        try {
          ...
          // Force a small delay then refresh to update UI state
          setTimeout(() => window.location.reload(), 1500);
        } catch (err) {
          toast.error("Payment verified, but account update failed. Contact support.", { id: toastId });
        }
      },
      () => {
        // Close Callback
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 pb-20 px-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className={`px-6 py-2 rounded-full border-2 flex items-center gap-2 ${userData?.tier === 'PRO' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
          {userData?.tier === 'PRO' ? <ShieldCheck size={16} /> : <ZapOff size={16} />}
          <span className="font-black uppercase text-[10px] tracking-widest">
            {userData?.tier === 'PRO' ? 'Pro Plan Active' : 'Free Plan'}
          </span>
        </div>
      </div>

      {userData?.tier !== 'PRO' && (
        <div className="bg-slate-900 rounded-[3rem] p-8 md:p-12 text-white overflow-hidden relative shadow-2xl border border-white/5">
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="text-center md:text-left">
              <h3 className="text-3xl font-black italic uppercase mb-2">Unlock Geo-Guard</h3>
              <p className="text-slate-400 font-medium max-w-md">
                Stop buddy-punching. Ensure staff are physically on-site before they can clock in.
              </p>
            </div>
            <button
              type="button"
              onClick={handleUpgrade}
              className="px-10 py-5 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black uppercase text-xs flex items-center gap-3 transition-all active:scale-95 shadow-lg shadow-blue-500/20"
            >
              <Crown size={18} /> Get Pro for ₦5,000
            </button>
          </div>
              ...
              {showMapPicker && (
                <div className="border rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
                  <div className="p-4 bg-slate-900 flex flex-col md:flex-row gap-2">
                    ...
                    <button type="button" onClick={handleSearch} className="px-6 py-2 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase">Search</button>
                  </div>
                  ...
                  <div className="p-4 bg-slate-50 flex justify-end gap-2">
                    ...
                    }} className="px-6 py-2 bg-emerald-600 text-white rounded-xl font-bold text-xs uppercase">Confirm Pin</button>
                  </div>
                </div>
              )}
                ...
                <div className="col-span-2 md:col-span-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full p-4 pl-12 bg-slate-50 rounded-2xl font-black text-lg focus:ring-2 focus:ring-blue-500 outline-none"
                    />
                  </div>
                </div>
